...
class InvoiceController extends Controller
{
	...
	public function indexView()
	{
		 ...
		 **/
		$data['result'] = $this->invoiceModel->getInvoiceView($id);
		...
		if (empty($data['result']['billing_id'])) {
			...
			$info = $this->invoiceModel->getBiller($data['result']['billing_id']);
			$info['logo'] = '';
		}
	...
	public function indexEdit()
	{
		 ...
		 * Get all User data from DB using User model 
		 **/
		$data['result'] = $this->invoiceModel->getInvoice($id);
		if (empty($data['result'])) {
			$this->url->redirect('invoices');
		}
	...
	{
		$data = $this->invoiceModel->getInvoiceView($id);
	 ...
	 * This method will be called to mail invoice reminder etc
	 **/
	public function indexMail()
	{
		...
		*/
		...
		$mailer->mail->AltBody = 'Hello,
		Your new invoice has been created. 
		Invoice Number - INV-' . str_pad($data['invoice'], 4, '0', STR_PAD_LEFT) . '
		...
		if ($mailer->sendMail()) {
			...
			$this->session->data['message'] = array('alert' => 'error', 'value' => 'Email failed to send.');
		}
		if ($this->commons->validateToken($this->url->post('_token'))) {
			$this->url->redirect('invoice/view&id=' . $data['invoice']);
		}
	...
	public function createPDFHTML($id)
	{
		 ...
		 * Get all User data from DB using Invoice model 
		 **/
		$result = $this->invoiceModel->getInvoiceView($id);
		...
		if (empty($result['billing_id'])) {
			...
			$info = $this->invoiceModel->getBiller($result['billing_id']);
     ...
     .remit{
		...
		<tbody>
		<tr class="inv-head">
		<td colspan="2" class="inv-address" style="vertical-align: middle;">
