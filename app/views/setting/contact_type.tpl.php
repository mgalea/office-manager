    ...
    <div class="panel panel-default">
        <div class="panel-head">
            ...
            <div class="panel-action">
                <a class="btn btn-success btn-sm" data-toggle="modal" data-target="#addContactsModel"><i class="icon-plus mr-1"></i> <?php echo $lang['settings']['text_new_contact_type']; ?></a>
            </div>  
        </div>
                ...
                <table class="table table-bordered table-striped datatable-table" width="100%">
                    <thead>
                        <tr class="table-heading">
                            ...
                            <th><?php echo $lang['settings']['text_contact_type_name']; ?></th>
                    ...
                    <tbody>
                        ...
                        <tr> 
                            ...
                            <td class="table-action">
                                <a class="btn btn-info btn-circle btn-outline btn-outline-1x edit-_s-type" data-toggle="tooltip" title="<?php echo $lang['common']['text_edit']; ?>" data-name="<?php echo $value['name'] ?>" data-description="<?php echo $value['description'] ?>" data-id="<?php echo $value['id'] ?>" data-status="<?php echo $value['status'] ?>">
...
<!-- ADD EDIT MODAL -->
<div class="modal fade" id="addContactsModel" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php echo $lang['settings']['text_new_contact_type']; ?></h5>
            ...
            <form action="<?php echo $action; ?>" method="post">
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-form-label"><?php echo $lang['settings']['text_contact_type_name']; ?></label>
                        <input type="text" class="form-control" name="name" placeholder="<?php echo $lang['settings']['text_contact_type_name']; ?>">
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('click', '.edit-expense-type', function () {
            var ele = $(this);
            $('#addContactsModel input[name="name"]').val(ele.data("name"));
            $('#addContactsModel textarea[name="description"]').val(ele.data("description"));
            $('#addContactsModel input[name="id"]').val(ele.data("id"));
            $('#addContactsModel select[name="status"]').val(ele.data("status"));
            $('#addContactsModel .modal-title').text('<?php echo $lang['settings']['text_edit_contact_type']; ?>');
            $('#addContactsModel').modal('show');
        });

        $('#addContactsModel').on('hidden.bs.modal', function (e) {
            $('#addContactsModel .modal-title').text('<?php echo $lang['settings']['text_new_contact_type']; ?>');
            $('#addContactsModel input').not( "[name='_token']" ).val('');
            $('#addContactsModel textarea').val('');
        });
