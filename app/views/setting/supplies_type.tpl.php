    ...
    <div class="panel panel-default">
        <div class="panel-head">
            ...
            <div class="panel-action">
                <a class="btn btn-success btn-sm" data-toggle="modal" data-target="#addSuppliesModel"><i class="icon-plus mr-1"></i> <?php echo $lang['settings']['text_new_supplies_type']; ?></a>
            </div>  
        </div>
                ...
                <table class="table table-bordered table-striped datatable-table" width="100%">
                    <thead>
                        <tr class="table-heading">
                            ...
                            <th><?php echo $lang['settings']['text_supplies_type_name']; ?></th>
...
<!-- ADD EDIT MODAL -->
<div class="modal fade" id="addSuppliesModel" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?php echo $lang['settings']['text_new_supplies_type']; ?></h5>
            ...
            <form action="<?php echo $action; ?>" method="post">
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-form-label"><?php echo $lang['settings']['text_supplies_type_name']; ?></label>
                        <input type="text" class="form-control" name="name" placeholder="<?php echo $lang['settings']['text_supplies_type_name']; ?>">
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('click', '.edit-expense-type', function () {
            var ele = $(this);
            $('#addSuppliesModel input[name="name"]').val(ele.data("name"));
            $('#addSuppliesModel textarea[name="description"]').val(ele.data("description"));
            $('#addSuppliesModel input[name="id"]').val(ele.data("id"));
            $('#addSuppliesModel select[name="status"]').val(ele.data("status"));
            $('#addSuppliesModel .modal-title').text('<?php echo $lang['settings']['text_edit_supplies_type']; ?>');
            $('#addSuppliesModel').modal('show');
        });

        $('#addSuppliesModel').on('hidden.bs.modal', function (e) {
            $('#addSuppliesModel .modal-title').text('<?php echo $lang['settings']['text_new_supplies_type']; ?>');
            $('#addSuppliesModel input').not( "[name='_token']" ).val('');
            $('#addSuppliesModel textarea').val('');
        });
